...
func runGota(api *GotaAPI, command string, apiPrefix string, parameters map[string]interface{}) ([]byte, error) {
	// set parameters
	vals := url.Values{}
...
func runGotaRaw(api *GotaAPI, requestUrl string) ([]byte, error) {
...
func (api *GotaAPI) GetMatchHistory(playerName string, heroId int, gameMode string, skill string, dateMin string, dateMax string,
	...
	if err == nil {
		err = json.Unmarshal(result, &retval)
	}
	return retval, err
}
...
func (api *GotaAPI) GetLeagueListing() (Heroes, error) {
	var retval Heroes = Heroes{}
	parameters := make(map[string]interface{})
	var err error = nil
	var result []byte
